...
namespace ShipManifest.Windows.Tabs
{
  internal static class TabConfig
  {
    ...
    internal static void Display(Vector2 displayViewerPosition)
    {
      ...
      // Log Debug to Ksp Error Log.
      _label = "Enable Logging to KSP log (Output.log)";
      _toolTip = "When On, Ship Manifest automatically logs to the KSP log.";
      _toolTip += "\r\nThis is a troubleshooting aid.";
      _guiLabel = new GUIContent(_label, _toolTip);
      SMSettings.KspLogDebug = GUILayout.Toggle(SMSettings.KspLogDebug, _guiLabel, GUILayout.Width(300));
      _rect = GUILayoutUtility.GetLastRect();
      if (Event.current.type == EventType.Repaint && _canShowToolTips)
        ToolTip = SMToolTips.SetActiveToolTip(_rect, GUI.tooltip, ref ToolTipActive, scrollX);

      // Enable Verbose Ksp Logging
      GUI.enabled = SMSettings.KspLogDebug;
      _label = "Enable Verbose Ksp Logging";
      _toolTip = "When On, Ship Manifest automatically logs detailed information to the Ksp log.";
      _toolTip += "\r\nThis is a troubleshooting aid.";
      _guiLabel = new GUIContent(_label, _toolTip);
      SMSettings.KspLogVerbose = GUILayout.Toggle(SMSettings.KspLogVerbose, _guiLabel, GUILayout.Width(300));
      _rect = GUILayoutUtility.GetLastRect();
      if (Event.current.type == EventType.Repaint && _canShowToolTips)
        ToolTip = SMToolTips.SetActiveToolTip(_rect, GUI.tooltip, ref ToolTipActive, scrollX);
      GUI.enabled = true;
